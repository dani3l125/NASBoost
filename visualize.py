...
# Number of clusters sampled in each zoom in step of the visualization.
CLUSTERS = 20
    ...
    for k in [10, 25, 50, 100]:
        for dataset in ['cifar10', 'cifar100', 'imagenet']:
            for distance in ['nasbot', 'path', 'adj', 'lev']:
                ...
                dist_matrix = np.load(f'distances/{distance}_dist.npy')
                acc_dist_matrix = np.load(f'distances/{dataset}_dist.npy')
                ...
                for zoom in [1, 2]:
                    sampled_clusters = np.random.randint(0, np.max(labels), CLUSTERS)
                    sampled_points_indexes = \
                        np.concatenate([np.argwhere(labels == sampled_clusters[i]) for i in range(CLUSTERS)]).T[0]
